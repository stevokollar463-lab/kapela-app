...
def zobraz_footer_tlacidla():
    ...
    if st.session_state.get(f"expand_faq_{page_id}", False):
        ...
        faq_otazky = [
            ...
            {"otazka": "Aká je minimálna doba rezervácie?", "odpoved": "Rezervácia musí byť uskutočnená minimálne 1 mesiac vopred."},
            {"otazka": "Ako sa počíta cena?", "odpoved": f"Rodinná oslava: {CENA_OSLAVA_HODINA} € / hod | Svadobný sprievod: {CENA_SPRIEVOD_ZAKLAD} € / 2 hod | Hranie pomedzi stoly: {CENA_STOLY_HODINA} € / hod + doprava {CENA_ZA_KM} € / km"}
        ]
    ...
    if st.session_state.get(f"expand_rec_{page_id}", False):
        st.markdown('<div class="expandable-section">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">⭐ Recenzie a hodnotenie</div>', unsafe_allow_html=True)

        recenzie_data = nacti_recenzie()
        if recenzie_data:
            st.markdown("### 💬 Posledné recenzie")
            for rec in recenzie_data[:5]:
                st.markdown(f"""
            ...
            st.info("Zatiaľ tu nie sú žiadne recenzie. Buď prvý 😊")

        st.markdown("<hr style='border-color: rgba(212,175,55,0.3); margin: 20px 0;'>", unsafe_allow_html=True)
        st.markdown("### ✍️ Pridaj recenziu")

        with st.form(f"nova_recenzia_{page_id}"):
            typ_mena = st.radio("Ako sa chceš reprezentovať?", ["❌ Anonymne", "✅ Pod mojim menom"], horizontal=True, key=f"radio_rec_{page_id}")

            meno = ""
            if typ_mena == "✅ Pod mojim menom":
                meno = st.text_input("Tvoje meno", placeholder="Napíš svoje meno...", key=f"input_meno_{page_id}").strip()
                if not meno:
                    meno = "Anonymný"
            else:
                meno = "Anonymný"

            hvezdicky = st.slider("Ako hodnotíš našu kapelu? (1-5 hviezd)", min_value=1, max_value=5, value=5, key=f"slider_rec_{page_id}")
            text = st.text_area("Tvoj komentár", placeholder="Napíš nám tvoj názor na naše vystúpenie...", height=120, key=f"textarea_rec_{page_id}")
    ...
    if st.session_state.get(f"expand_kon_{page_id}", False):
        ...
        st.markdown("""
            <div class="kontakt-info"><div class="kontakt-label">☎️ Telefón</div><div class="kontakt-value">0944 757 122</div></div>
            <div class="kontakt-info"><div class="kontakt-label">📧 Email</div><div class="kontakt-value">parobciovcanske@gmail.com</div></div>
            <div class="kontakt-info"><div class="kontakt-label">📍 Mesto</div><div class="kontakt-value">Obec Ovčie, Slovensko</div></div>
            <div style="text-align:center; margin-top:15px;">
                <a href="https://www.instagram.com/ovcanske_parobci/" target="_blank"
                   style="display:inline-block; padding:10px 20px; background-color:#d4af37; color:black; text-decoration:none; border-radius:8px; font-weight:bold;">
...
# ✅ RECENZIE ODSTRÁNENÉ Z HORNÉHO MENU
menu = st.radio(
    "NAVIGÁCIA",
...
if menu == "🎸 Rezervácia":
                ...
                if supabase:
                    try:
                        ...
                        if res.data:
                            ...
                            st.error("Chyba: Dáta sa nepodarilo zapísať.")
                    except Exception as e:
    ...
    st.markdown("""
        <div class="o-nas-box">
            ...
            <div class="o-nas-text">
                <p>Sme ľudová kapela založená v roku <strong>2020</strong>.</p>
    ...
    if not st.session_state['auth']:
        ...
        with t3:
            ...
            st.info("Sekcia nahrávania médií ostáva nezmenená podľa pôvodného kódu.")

        with t4:
            st.subheader("➕ Manuálne pridať akciu")
            st.info("Sekcia pridania akcie ostáva nezmenená podľa pôvodného kódu.")
