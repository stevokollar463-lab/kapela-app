...
if menu == "🎸 Rezervácia":
    ...
    # --- OBSADENOSŤ - zobrazenie obsadených termínov ---
    db_aktualne = nacti_data()
    obsadene_datumy = [a['datum'] for a in db_aktualne if a.get('stav') == 'schvalene']

    if obsadene_datumy:
        datumy_html = " ".join(
            [f'<span class="obsadenost-datum">❌ {d}</span>' for d in sorted(obsadene_datumy)]
        )
        st.markdown(f"""
        <div class="obsadenost-box">
            <span style="font-size: 1.1rem; color: #ff4444; font-weight: bold;">🚫 Obsadené termíny:</span><br><br>
            {datumy_html}
        </div>
        """, unsafe_allow_html=True)
    ...
    with st.form("main_booking"):
        ...
        # Live warning pri výbere obsadeného dátumu
        datum_obsadeny = str(datum) in obsadene_datumy
        if datum_obsadeny:
            st.error("⚠️ Tento dátum je už OBSADENÝ! Zvoľte prosím iný termín.")
        ...
        if st.form_submit_button("ODOSLAŤ REZERVÁCIU S TOUTO CENOU"):
            if datum_obsadeny:
                st.error("❌ Tento termín je už obsadený. Vyberte prosím iný dátum.")
