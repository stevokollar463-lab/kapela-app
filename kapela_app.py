...
def zobraz_footer_tlacidla(is_recenzie_page=False):
    ...
    # ⭐ RECENZIE TLAČIDLO ODSTRÁNENÉ (už sa pridávajú len hore v Recenzie sekcii)
...
if menu == "🎸 Rezervácia":
    ...
    # PRIDÁVANIE RECENZIE LEN HORE
    with st.expander("➕ Pridať recenziu", expanded=False):
        with st.form("nova_recenzia_hore"):
            typ_mena_top = st.radio(
            ...
            meno_top = "Anonymný"
            if typ_mena_top == "✅ Pod mojim menom":
                meno_in = st.text_input("Tvoje meno", placeholder="Napíš svoje meno...", key="input_meno_top")
                if meno_in.strip():
                    meno_top = meno_in.strip()

            hvezdicky_top = st.slider("Hodnotenie (1-5)", 1, 5, 5, key="slider_rec_top")
            text_top = st.text_area("Tvoj komentár", placeholder="Napíš nám tvoj názor...", height=120, key="textarea_rec_top")

            if st.form_submit_button("🚀 ODOSLAŤ RECENZIU"):
                if not text_top or len(text_top.strip()) < 5:
                    st.warning("⚠️ Napíš prosím aspoň pár slov do komentára!")
                else:
                    nova_recenzia_top = {
                        "id": str(datetime.now().timestamp()),
                        "meno": meno_top,
                        "hvezdicky": hvezdicky_top,
                        "text": text_top.strip(),
                    ...
                    if supabase:
                        try:
                            res = supabase.table("recenzie").insert(nova_recenzia_top).execute()
                            if res.data:
    ...
    if recenzie:
        ...
        for rec in recenzie:
            st.markdown(f"""
            <div class="rec-card" style="background: rgba(0, 0, 0, 0.8); border: 2px solid #d4af37; padding: 15px; border-radius: 12px; margin: 12px 0;">
    ...
    # ... tvoja administrácia ostáva rovnaká (bez zmeny)
    st.title("🔐 Administrácia")
    st.info("Admin časť ponechaná bezo zmeny.")
